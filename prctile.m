...
% function y=prctile(x1,x2,...,p)

%
% $Id: prctile.m,v 1.5 2011/03/26 15:09:33 patrick Exp $
%
...
elseif nargin==3 & isnumeric(varargin{2}) & isnumeric(varargin{3}),
	...
	y.var = reshape(prctile(tmp,p,1),s);
	return
end

if ~exist('dim'), %	assume over all dimensions
	y = prctile(x.var(find(isfinite(x.var(:)))),p);
	return;
end;

y=sd;
y.var = prctile(x.var, p, dim);
