function y=kurtosis(varargin)
% function y=kurtosis(x,flag)
% function y=kurtosis(x,flag,dim)
% function y=kurtosis(x1,x2,...,flag,dim)

%
% $Id: kurtosis.m,v 1.9 2011/03/26 15:09:33 patrick Exp $
%
...
elseif nargin==3 & isnumeric(varargin{2}) & isnumeric(varargin{3}),
	...
	y.var = reshape(kurtosis(tmp,flag,1),s);
	return
end

if ~exist('dim'), % assume over all dimensions 
	y = kurtosis(x.var(find(isfinite(x.var(:)))));
	return;
end;

y=sd;
y.var = kurtosis(x.var,flag,dim);
