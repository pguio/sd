function h=mesh(f,nrnc)
% function h=mesh(f,nrnc)

%
% $Id: mesh.m,v 1.4 2011/03/26 15:09:33 patrick Exp $
%
...
for j=1:nj,
	...
	switch length(f.dims),
		case 2, hh(j)=mesh(f.dims{1:2} , f.var'); 
		case 3, hh(j)=mesh(f.dims{2:3}, squeeze(f.var(j,:,:))');
		case 4, idims=[];
			for i=2:4, 
				if length(f.dims{i})~=1, idims=[idims, i]; end
			end
			hh(j)=mesh(f.dims{idims}, squeeze(f.var(j,:,:,:))');
	end
