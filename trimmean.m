function y=trimmean(varargin)
% function y=trimmean(x,p)
% function y=trimmean(x,p,dim)
% function y=trimmean(x1,x2,...,p,dim)

%
% $Id: trimmean.m,v 1.10 2011/03/26 15:09:33 patrick Exp $
%
...
elseif nargin==3 & isnumeric(varargin{2}) & isnumeric(varargin{3}),
  ...
  tmp = zeros([nargin-1, s]);
	...
	y.var = reshape(trimmean(tmp,p,1),s);
	return
end

if ~exist('dim'), % assume over all dimensions
	y = trimmean(x.var(find(isfinite(x.var(:)))),p);
	return;
end;

y = sd;
y.var = trimmean(x.var,p,dim);
