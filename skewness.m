function y=skewness(varargin)
% function y=skewness(x,flag)
% function y=skewness(x,flag,dim)
% function y=skewness(x1,x2,...,flag,dim)

%
% $Id: skewness.m,v 1.10 2015/09/24 13:52:56 patrick Exp $
%
...
elseif nargin==3 & isnumeric(varargin{2}) & isnumeric(varargin{3}),
	...
	y.var = reshape(skewness(tmp,flag,1),s);
	return
end

if ~exist('dim'), % assume over all dimensions
	y = skewness(x.var(isfinite(x.var(:))));
	return;
end;

y = sd;
y.var = skewness(x.var,flag,dim);
