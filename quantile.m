function y=quantile(varargin)
% function y=quantile(x,p)
% function y=quantile(x,p,dim)
% function y=quantile(x1,x2,...,p)

%
% $Id: quantile.m,v 1.13 2011/03/26 15:09:33 patrick Exp $
%
...
elseif nargin==3 & isnumeric(varargin{2}) & isnumeric(varargin{3}),
	...
	y.var = reshape(quantile(tmp,p,1),s);
	return
end

if ~exist('dim'), %	assume over all dimensions
	y = quantile(x.var(find(isfinite(x.var(:)))),p);
	return;
end;

y=sd;
y.var = quantile(x.var, p, dim);
