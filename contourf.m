function h=contourf(f,nrnc,clim,colorbarflag)
% function h=contourf(f,nrnc,clim,colorbarflag)

%
% $Id: contourf.m,v 1.7 2011/03/26 15:09:33 patrick Exp $
%
...
for j=1:nj,
	...
	switch length(f.dims),
		case 2, [c{j},h{j}]=contourf(f.dims{1:2}, f.var', clim);
		case 3, [x,y]=meshgrid(f.dims{2:3}); 
		[c{j},h{j}]=contourf(x,y, squeeze(f.var(j,:,:))');
		case 4, idims=[];
			for i=2:4, 
				if length(f.dims{i})~=1, idims=[idims, i]; end
			end
			[c{j},h{j}]=contourf(f.dims{idims}, squeeze(f.var(j,:,:,:))', clim);
	end
